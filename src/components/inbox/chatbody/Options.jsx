...
export default function Options({ existedMessage }) {
	// local states
	...
	const handleSendMessage = (e) => {
		...
		const partner =
			existedMessage.sender.email !== user.email
				? existedMessage.sender
				: existedMessage.receiver;

		const data = {
			participants: `${user.email}-${partner.email}`,
			users: [user, partner],
			message,
